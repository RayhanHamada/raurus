...
- **Route options** — `routes.ts` takes `RouteOptions` with optional `metadata` and `storage` adapter objects. `metadataAdapter` and `storageAdapter` are now optional on `CreateRuntimeOptions` as well; individual routes guard with the `checkMetadata` / `checkStorage` Elysia macros and return `501 Not Implemented` when the relevant adapter is missing.
...
- **Storage menu routes** — `GET /presigned-url`, `GET /presigned-download-url`, `POST /upload-asset`, and `DELETE /asset/:assetKey` each guard the corresponding optional method on the storage adapter. When a method is absent on the adapter, the route returns `501 Not Implemented` (not `400`) via inline `status(501, { message: "Error", error: "..." })` calls.
...
- Pass adapter dependencies to `routes()` via `RouteOptions`, not through Elysia state — both `metadata` and `storage` are optional; routes guard missing adapters with the `checkMetadata` / `checkStorage` macros
...
- `RaurusAsset` from `@raurus/core` is `ArrayBuffer` only; the `POST /upload-asset` body is validated by `m.UploadAssetBodySchema` and the route currently only checks that the storage adapter exists and implements `createPresignedUploadUrl` — it does not yet read or call `uploadAsset` on the adapter (the in-memory adapter no-ops any future call)
